...
* **Шанс появления битого символа: 0.46% = 1/217**
...
Конфигурации, подразумевающие использование передатчика и приемника на обеих сторонах, обеспечивая связь в обе стороны. Данная связь позволяет использовать обратную связь для обеспечения более высоких скоростей, стабильности соединения и целостности передаваемых данных.

###Обратная связь

Для развития большой скорости передачи информации, необходимо улучшить стабильность и целостность получаемых данных, ибо из-за большой скорости, данные **ОЧЕНЬ ЧАСТО** бьются, и получается полная каша. Поэтому на приеме необходимо проверить целостность полученных пакетов данных, или же добавить в них **контрольную суммы**. **Контрольная сумма** - это **специальный байт информации**, который получается при использовании определенного алгоритма (**Например:** суммирование каких-то определенных байтов пакета), по которому можно определить целостность полученного пакета. Все, что делает приемник, это применяет этот определенный алгоритм и сравнивает его со значением, которое он получил. Да, это не гарантирует получение только целых пакетов, но зато отсеивает их в очень огромных количествах. В 

## Свойства связи:

* **Максимальная достигнутая скорость: 38.4 Кбит/сек**      **!! НЕ ЯВЛЕТСЯ ПРЕДЕЛОМ !!**

* **Максимальная фактическая скорость: +- 37.6 Кбит/сек**   **!! НЕ ЯВЛЕТСЯ ПРЕДЕЛОМ !!**

* **Шанс появления битого символа: 0.0013(8)% = 1/72000**
...
Конфигурация для передачи изображения на высокой скорости (с обратной связью)

#### Transmitter_PhotoDisplay

Код для **передатчика**. Работает так же, как и код [передатчика](https://github.com/NoobieNightCoder/FSO_Arduino/edit/main/README.md#transmitter_photodisplay) для передачи изображения без обратной связи, **за исключением того**, что он будет отправлять **один и тот же пакет** до тех пор, пока не примет на свой приемник нужный сигнал **"ОК"**, сигнализирующий об успешной отправке пакета, от **приемника**. Структура отправляемого пакета следующая:

```bash
[X_COORDINATE, Y_COORDINATE, HEX_COLOR, BYTE_SUM]
```

**BYTE_SUM** - Контрольная сумма пакета. Получается путем сложения **X** координаты с **Y** координатой передаваемого пикселя:

```bash
BYTE_SUM = X_COORDINATE + Y_COORDINATE
```
...
Код для **приемника**. Работает так же, как и код [приемника](https://github.com/NoobieNightCoder/FSO_Arduino/edit/main/README.md#receiver_photodisplay) для передачи изображения без обратной связи, **за исключением того**, что он проверяет целостность получаемого пакета и отправляет соответствующие нужные сигналы **передатчику**: **"ОК"** или **"NO"**.

P.S. Сигнал "OK" можно поменять на 1 любой символ **(!! Но тогда будет выше вероятность прохождения битого пакета !!)** и вообще убрать отправку сигнала "NO" для оптимизации.
