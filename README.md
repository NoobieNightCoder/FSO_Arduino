...
Код для **передатчика**. Работает так же, как и код [передатчика](https://github.com/NoobieNightCoder/FSO_Arduino?tab=readme-ov-file#transmitter_photodisplay) для передачи изображения без обратной связи, **за исключением того**, что он будет отправлять **один и тот же пакет** до тех пор, пока не примет на свой приемник нужный сигнал **"ОК"**, сигнализирующий об успешной отправке пакета, от **приемника**. Структура отправляемого пакета следующая:
...
Код для **приемника**. Работает так же, как и код [приемника](https://github.com/NoobieNightCoder/FSO_Arduino?tab=readme-ov-file#receiver_photodisplay) для передачи изображения без обратной связи, **за исключением того**, что он проверяет целостность получаемого пакета и отправляет соответствующие нужные сигналы **передатчику**: **"ОК"** или **"NO"**.
